...
#define MAP_ITER_NEXT(m, it, key, val) (_collections_map_iter_check(&(it), (m).cap, (m).b_len) ? ((key) = (m).keys[(it).i][(it).j], (val) = (m).vals[(it).i][(it).j], (it).j + 1 < (m).b_len[(it).i] ? ((it).j++) : ((it).i++, (it).j = 0), 1) : 0)
...
char _collections_map_iter_check(struct map_iter *it, size_t cap, size_t *b_len);
#endif
